...
public class GameUserRegisterController {
	...
	static 
	{
		...
		msg.put(RegisterResponseBean.USERNAME_HAS_REGISTER, "用户名已经注册");
		msg.put(RegisterResponseBean.USERNAME_HAS_SPECIACHAR, "用户名不能包含特殊字符");
		msg.put(RegisterResponseBean.USERNAME_TOO_SHORT, "用户名太短");
		msg.put(RegisterResponseBean.PASSWORD_HAS_SPECIACHAR, "密码不能包含特殊字符");
		msg.put(RegisterResponseBean.PASSWORD_TOO_SHORT, "密码太短");
		msg.put(RegisterResponseBean.FAILED_CODE, "一般性错误");
	}
        	...
        	if (g_user.getPassword().equals(g_user.getRe_password()))
        	{
        		...
        		if (gameUserManager.insert(g_user))
        		{
        			...
        			if (bean != null)
        			{
                		...
                		{
                			model.addAttribute("errorMessage", msg.get(bean.getCode()));
                		}

        			}
        			else
            		{
            			model.addAttribute("errorMessage", msg.get(RegisterResponseBean.FAILED_CODE));
            		}
        		}
        		else
        		{
	...
	public String downloadPage(HttpServletRequest request, HttpServletResponse response, String gameid, Model model)
	{
		if (!StringUtility.hasEmpty(gameid))
		{
			BusiGame g = gameManager.getGameById(gameid);
			
			if (g != null)
			{
		        model.addAttribute("__content_page", "/game_download"); 
		        return "/layout/g_user_register";
			}
		}
		return "/";
