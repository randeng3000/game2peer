...
@Service
public class UserGallary implements ApiBean {
	@Autowired
	...
	public Object requestProcess(HttpServletRequest request, HttpServletResponse response, String id, User user,
		...
		if (user != null)
		{
			...
			if (!StringUtility.hasEmpty(kind, page))
			{
				return userManager.getImages(user.getId(), Integer.valueOf(kind), Integer.valueOf(page) - 2);
			}
		}
		return null;
	}
