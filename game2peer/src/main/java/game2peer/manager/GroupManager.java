...
public class GroupManager {
	...
	public Map<String, AccessRight> getGroupAccessRight(String groupId)
	{
		...
		for (int i = 0; i < l.size(); i++)
		{
			...
			value.setGet(l.get(i).getGet());
			value.setPut(l.get(i).getPut());
			value.setDel(l.get(i).getDel());
			value.setModi(l.get(i).getModi().intValue());
			value.setModi(l.get(i).getModi().intValue());
			value.setOper(l.get(i).getOper().intValue());
