...
public class GroupManager {
	...
	public Map<String, AccessRight> getGroupAccessRight(String groupId)
	{
		...
		for (int i = 0; i < l.size(); i++)
		{
			...
			value.setGet((l.get(i).getGet().intValue() > 0) ? 1 : 0);
			value.setPut((l.get(i).getPut().intValue() > 0) ? 1 : 0);
			value.setDel((l.get(i).getDel().intValue() > 0) ? 1 : 0);
			value.setModi((l.get(i).getModi().intValue() > 0) ? 1 : 0);
